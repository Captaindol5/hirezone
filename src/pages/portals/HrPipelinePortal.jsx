        ...
        <main className="space-y-6">
          ...
          {activeTab === 'candidates' && (
            <section className="space-y-6">
              ...
              <div className="rounded-[28px] border border-[var(--border-color)] bg-white/70 p-5 shadow-[var(--shadow-soft)] dark:bg-slate-900/80">
                <div className="mb-4">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Candidate records</p>
                  <h2 className="mt-2 text-[42px] font-black leading-tight tracking-[-0.05em] text-[var(--text-headers)]">Live candidate profiles</h2>
                </div>
                ...
                <div className="space-y-3">
                        ...
                        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-[var(--text-muted)]">{job.title} · {candidate.email}</p>
                            </div>
                          </div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className={`rounded-full px-3 py-1 text-[11px] font-semibold ${candidate.hasSubmittedFeedback ? 'bg-emerald-500/10 text-emerald-700' : 'bg-amber-500/10 text-amber-700'}`}>
                  ...
                  {data.jobs.every((job) => !job.candidates?.length) && (
                    <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-[var(--border-color)] bg-[var(--bg-secondary)]/50 p-8 text-center">
                      ...
                      <p className="text-xs text-[var(--text-muted)]">Create your first candidate above to see them here</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="grid gap-4 xl:grid-cols-4">
                {selectedJob?.stages.map((stage, index) => (
                  <div key={stage.id} className="min-h-[260px] rounded-2xl border border-[var(--border-color)] bg-[var(--bg-secondary)] p-3">
                    ...
                    <div className="space-y-3">
                      {(selectedJob.candidates || []).filter((candidate) => candidate.stage === stage.id).length === 0 ? (
                        <div className="rounded-2xl border border-dashed border-[var(--border-color)] p-4 text-center text-sm text-[var(--text-muted)]">No candidates in this stage</div>
                      ) : (
                          ...
                          .map((candidate) => (
                            <div key={candidate.id} className="rounded-2xl border border-[var(--border-color)] bg-white/70 p-3 shadow-sm transition-all duration-200 hover:shadow-md dark:bg-slate-800/80">
                              ...
                              <div className="mt-3 flex items-center justify-between gap-2">
                                <button onClick={() => advanceCandidate(candidate.id, index)} disabled={readOnly || !canAdvance(candidate)} className={`inline-flex items-center gap-1 rounded-xl px-3 py-2 text-xs font-semibold text-white ${readOnly || !canAdvance(candidate) ? 'cursor-not-allowed bg-slate-300 dark:bg-slate-700' : 'bg-orange-500'}`}>
                                  {readOnly ? 'View only' : 'Advance'}
                                  {!readOnly && <ChevronRight size={14} />}
                                </button>
